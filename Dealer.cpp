...
void Dealer::changeDeck() {
	...
	for (int i = 0; i < cardDeck; i++) { //카드 저장 및 셔플
		for (int j = 1; j <= 13; j++) {
...
void Dealer::calculateDealerCard(Card card) { //카드 점수 계산
	if (card.getValue() == 1) {
		sum += 11;
		aCard++;
	}
...
Card Dealer::drawCardOnCardList() { //맨 뒷장 pop
...
void Dealer::giveCard(bool dealerCardOpened) {
	for (vector<Seat*>::iterator player = players.begin(); player != players.end(); player++) {
		...
		(*player)->notifyMyCard(card); //자기 카드 확인
		for (vector<Seat*>::iterator other = players.begin(); other != players.end(); other++) {
			if (player == other) continue;

			(*other)->notifyOtherPlayerCard(card); //다른 사람 카드 확인, 뒤에 있는 거 이용해서 하심 됩니다.
		}
	}
...
void Dealer::playDealerTurn() {
	...
	while (sum < 16) {
		Card card = drawCardOnCardList();
		calculateDealerCard(card); //합계가 16보다 낮으면 계속 뽑기
		for (vector<Seat*>::iterator player = players.begin(); player != players.end(); player++) {
			//뽑고 난 이후에 다시 공개
			(*player)->notifyDealerCard(card);
		}
...
Dealer::Dealer(int gameRound, int cardDeck, int playerCount) {
	...
	players.push_back(new Seat(new Student())); //처음꺼는 학생 넣고
	for (int i = 1; i < playerCount; i++) { //나머지는 더미, i=0부터 player-1까지 넣어서 플레이어 순서 바꿔봄.
		players.push_back(new Seat(new DummyPlayer()));
	//	players.push_back(new Seat(new Student()));
	}
	//players.push_back(new Seat(new Student())); //테스트 삼아 마지막에 넣어보기
}
